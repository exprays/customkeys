...
<!DOCTYPE html><html><body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:40px 20px">
  <h2 style="color:#4f46e5">You've been invited to %s on Nano</h2>
  <p>%s has invited you to join their organization on Nano — the secrets & config manager.</p>
  ...
  <p style="color:#6b7280;font-size:14px">This invite expires in 7 days. If you did not expect this, you can ignore this email.</p>
	...
	return c.send(ctx, to, fmt.Sprintf("You've been invited to %s on Nano", orgName), html)
}
...
<!DOCTYPE html><html><body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:40px 20px">
  ...
  <p>Nano will retry automatically with exponential backoff. Log in to manually trigger a rotation or review the rotation history.</p>
</body></html>`, secretKey, secretKey, envName, errMsg)
	return c.send(ctx, to, fmt.Sprintf("[Nano] Rotation failed for %s in %s", secretKey, envName), html)
}
...
<!DOCTYPE html><html><body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:40px 20px">
  ...
  <p style="color:#6b7280;font-size:14px">This request expires in 24 hours.</p>
	...
	return c.send(ctx, to, fmt.Sprintf("[Nano] Approval needed for %s in %s", secretKey, envName), html)
}
